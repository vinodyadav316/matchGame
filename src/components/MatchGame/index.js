...
class MatchGame extends Component {
  ...
  renderContainerImage = id => {
    imagesList.filter(eachItem => eachItem.imageUrl === id)
  }
    ...
    return (
      <div className="app-container">
        <Navbar
          ...
          renderNavContainer={this.renderNavContainer()}
        />
        <div className="card-container">
          <div className="image-container">{this.renderContainerImage()}</div>
          <TabItem changeActive={this.isActive} />
          <ul className="un-ordered-list">
            {imagesList.map(item => (
              <ImageItems eachItem={item} key={item.id} isInitial={isInitial} />
            ))}
          </ul>
        </div>
      </div>
